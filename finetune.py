...
train_generator = ImageDataGenerator(train_file, 
                                     horizontal_flip = True, shuffle = False)
val_generator = ImageDataGenerator(val_file, shuffle = False,testData=True) 
...
def generateTestResult():
  ...
  for _ in range(val_batches_per_epoch):
      it+=1
      print it
      batch_tx= val_generator.next_batch(batch_size,testData=True)
...
def generFeatureTest():
  ...
  size_i=0
  sess=tf.Session()
  checkpoint_name = os.path.join(checkpoint_path, 'model_epoch'+str(45)+'.ckpt')
  saver.restore(sess,checkpoint_name)
